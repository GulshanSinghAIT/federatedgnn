...
- **`src/components/ui/` is a full shadcn-style library** (Button, Badge, Table, Select, Tabs/TabsSubtle, Tooltip, Slider, Switch, Dialog, Dropdown, Accordion, …). Prefer these over hand-rolled controls. They depend on `@/lib/shape-context` + `surface-context` + `icon-context`, which all have **safe defaults** (no provider required).
