...
export default function TreatmentPanel({ treatments, diagnoses }: Props) {
    return (
        <div className="glass-card p-4">
            <h3 className="text-xs font-medium text-text-muted uppercase tracking-wider mb-3">Treatment Recommendations</h3>
                ...
                {diagnoses.slice(0, 3).map(d => {
                    ...
                    return (
                        <div key={d.disease_id}>
                            <h4 className="text-xs text-node-disease font-medium mb-1">For: {d.disease_name}</h4>
                            <div className="space-y-1">
                                {tList.slice(0, 3).map(t => (
                                    <div key={t.treatment_id} className="flex items-center gap-2 text-xs p-2 bg-bg-tertiary/30 rounded">
                                        <span className="text-node-treatment">{typeIcon(t.type)}</span>
                                        <span className="flex-1">{t.treatment_name}</span>
                                        {evidenceBadge(t.evidence_level)}
                                        <span className="text-text-muted">{(t.efficacy * 100).toFixed(0)}%</span>
                                        {t.contraindications.length > 0 && (
                                            <span className="text-accent-red" title={t.contraindications.join(', ')}>
                                                <AlertCircle size={12} />
                                            </span>
                                        )}
                                    </div>
