  ...
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <AnimatePresence initial={false}>
        {isOpen && (
          <AccordionPrimitive.Content forceMount {...props} render={<motion.div ref={ref} className={cn("overflow-hidden", className)} initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} transition={{ ...spring.moderate, bounce: 0 }} onUpdate={() => {
                                groupCtx?.remeasure();
                              }} onAnimationComplete={() => {
                                groupCtx?.remeasure();
