  ...
  ({ label, checked, onToggle, disabled = false, thumbTransition, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        >
          <SwitchPrimitive.Thumb render={<motion.span className="absolute top-0 left-0 block rounded-full bg-white shadow-sm" initial={false} style={{ x: motionX }} animate={{
                                y: thumbY,
                                width: thumbWidth,
                                height: thumbHeight,
                              }} transition={hasMounted.current ? (thumbTransition ?? spring.moderate) : { duration: 0 }} />}></SwitchPrimitive.Thumb>
        </SwitchPrimitive.Root>
